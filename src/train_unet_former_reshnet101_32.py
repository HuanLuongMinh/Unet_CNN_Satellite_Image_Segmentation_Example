"""
Training script for UNetFormer (decoder GLA transformer) — encoder
resnext101_32x16d.fb_swsl_ig1b_ft_in1k, loss = CrossEntropy thuần (không Dice),
BASE_LR = 1e-5.

File này độc lập với src/train_cnn_resnext101_32.py và src/train_cnn.py — chỉ
khác model import (decoder UNetFormer thay vì decoder CNN), toàn bộ pipeline
DDP/AMP/checkpoint/resume/early-stopping/visualization giữ nguyên để các thí
nghiệm decoder CNN cũ không bị ảnh hưởng khi chạy lại.

Usage (Kaggle, 2x T4):
    torchrun --nproc_per_node=2 src/train_unet_former_reshnet101_32.py --config configs/unet_former_reshnet101_32/luot1_500.yaml
    torchrun --nproc_per_node=2 src/train_unet_former_reshnet101_32.py --config configs/unet_former_reshnet101_32/luot1_500.yaml --dry-run
    torchrun --nproc_per_node=2 src/train_unet_former_reshnet101_32.py --config configs/unet_former_reshnet101_32/luot1_500.yaml --resume <work_dir>/latest_checkpoint.pth
"""
...
from src.models.unet_former_reshnet101_32 import build_model
